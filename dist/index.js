...
var plyr_polyfilled = createCommonjsModule(function (module, exports) {
...
var Player = /*#__PURE__*/function (_React$Component) {
  inherits(Player, _React$Component);

  function Player() {
    classCallCheck(this, Player);

    return possibleConstructorReturn(this, getPrototypeOf(Player).apply(this, arguments));
  }

  createClass(Player, [{
    key: "componentDidMount",
    value: function componentDidMount() {
      this.player = new plyr_polyfilled('.js-plyr', this.props.options);
      this.player.source = this.props.sources;
    }
  }, {
    key: "componentWillUnmount",
    value: function componentWillUnmount() {
      var destroy = function destroy(players) {
        var play;
      ...
      this.player && this.player.length > 0 && destroy(this.player);
    }
  }, {
    key: "render",
    value: function render() {
...
var VideoWrapper = function VideoWrapper(_ref) {
  var item = _ref.item,
      onClickHandler = _ref.onClickHandler,
      index = _ref.index;
  return React__default.createElement("div", {
...
var SlickText = function SlickText(_ref) {
  var item = _ref.item,
      index = _ref.index,
  ...
  return React__default.createElement("div", {
    ...
    className: "title"
  }, item.title, " ", item.type === 'video' ? React__default.createElement("span", {
    className: "play-text"
  }, " (Click to play)") : ''), React__default.createElement("span", {
    className: "counter"
  }, ' ', index + 1, "/", totalCount));
};
...
var GallerySlider = function GallerySlider(_ref) {
  var settings = _ref.settings,
      sliderData = _ref.sliderData;
  // const { onChange, imageUrlsList, settings } = props;
  // const [index, setIndex] = useState(0);
  var sliderRef = React.useRef(null);
  var playerRef = React.useRef(null);

  var onSliderChange = function onSliderChange(newIndex) {
    // setClickable(true);
    // setIndex(newIndex);
    // onChange(newIndex);
    console.log(sliderRef);
    console.log(playerRef);
    console.log("New index val ".concat(newIndex)); // playerRef.current && playerRef.current.player.fullscreen.enter();
    // if (sliderRef.innerSlider) {
    //     sliderRef.innerSlider.list.setAttribute('tabindex', 0);
    //     sliderRef.innerSlider.list.focus();
    // }

    playerRef.current && playerRef.current.player.pause();
  };

  var onClickHandle = function onClickHandle(index) {
    console.log("Slider item has been clicked ".concat(index));
  };

  var beforeChangeSlide = function beforeChangeSlide(oldIndex, newIndex) {
    // setClickable(false);
    // console.log(sliderRef);
    // console.log(playerRef);
    // playerRef.curent.player && playerRef.curent.player.pause();
    // playerRef.current && playerRef.current.player.fullscreen.exit();
    console.log('Before change called ', oldIndex, newIndex);
  };
  ...
  var combinedSettings = lodash.merge({}, settings, SLICK_DEFAULT_SETTINGS, methodDefaults);
  return React__default.createElement("div", {
    className: "media-slider"
  }, React__default.createElement(Slider, _extends_1({
    ref: sliderRef
  }, combinedSettings), sliderData && sliderData.length && lodash.map(sliderData, function (item, index) {
    return item.type === 'image' ? React__default.createElement(React__default.Fragment, {
      key: index
    }, React__default.createElement(ImageWrapper, {
      item: item,
      index: index,
      onClickHandler: onClickHandle,
      count: sliderData.length
    }), React__default.createElement(SlickText, {
      key: item.title,
      item: item,
      index: index,
      ...
      key: index
    }, React__default.createElement(VideoWrapper, {
      item: item,
      index: index,
      onClickHandler: onClickHandle,
      count: sliderData.length
...
var GallerySlider$1 = /*#__PURE__*/function (_PureComponent) {
  inherits(GallerySlider$1, _PureComponent);

  function GallerySlider$1(props) {
    var _this;

    classCallCheck(this, GallerySlider$1);

    // const {} = props;
    _this = possibleConstructorReturn(this, getPrototypeOf(GallerySlider$1).call(this, props));
    _this.state = {
      sliderData: _this.filterSliderData(),
      isCodeFilterAvailable: props.isCodeFilterAvailable,
      isPoprertyFilterAvailable: props.isPoprertyFilterAvailable,
      slideCounter: 0
    };
    return _this;
  }

  createClass(GallerySlider$1, [{
    key: "filterSliderData",
    value: function filterSliderData() {
      var _this$props = this.props,
          sliderData = _this$props.sliderData,
          isCodeFilterAvailable = _this$props.isCodeFilterAvailable,
          isPoprertyFilterAvailable = _this$props.isPoprertyFilterAvailable,
          property = _this$props.property,
          code = _this$props.code;

      var filterPropertyWithCode = function filterPropertyWithCode(sliderData) {
        var _find = lodash.find(sliderData, lodash.matchesProperty('propertyId', property)),
            galleryData = _find.galleryData;

        var galleryFiltered = lodash.find(galleryData, lodash.matchesProperty('galleryCode', code)); //     if(item.propertyId === property && item.galleryData) {
        //         item.galleryData.filter((galleryItem) => {
        //            return galleryItem.galleryData === code
        //        })
        //    } else {
        //        console.log('error processing data');
        //    }
        // const codeFiltered = propertyFiltered.filter((item) => {
        //     (item.galleryData) && item.galleryData.filter((item) => {
        //         return item.galleryCode === code
        //     })
        // })
      ...
      var _ref = property && code ? filterPropertyWithCode(sliderData) : [],
          items = _ref.items;
    ...
    value: function render() {
      ...
      return React__default.createElement(GallerySlider, {
  ...
  return GallerySlider$1;
}(React.PureComponent);

GallerySlider$1.propTypes = {
  code: PropTypes.string,
  isCodeFilterAvailable: PropTypes.bool,
  isPoprertyFilterAvailable: PropTypes.bool,
  property: PropTypes.string,
  settings: PropTypes.object,
  sliderData: PropTypes.arrayOf(PropTypes.object)
};
GallerySlider$1.defaultProps = {
  isCodeFilterAvailable: true,
  isPoprertyFilterAvailable: true,
  property: '',
  code: ''
};

module.exports = GallerySlider$1;
